...
chroma_path = "/var/www/html/moodle/local/chatbot/chromadb/chromadb_last/content/drive/MyDrive/RAG_heading_chunks_MOODLE"
...
def format_output(answer: str) -> str:
    """
    Gelen Markdown cevabı HTML'e çevirir.
    Başlık seviyelerini, listeleri, kalın metni vs. direkt işler.
    """
    # markdown kütüphanesini kullanarak HTML üret
    raw_html = markdown.markdown(
        answer,
        extensions=['extra', 'sane_lists', 'smarty']
    )
    # İsterseniz dış bir div ile sarıp CSS sınıfı ekleyin
    return f'<div class="chatbot-response">{raw_html}</div>'


output = generate_answer(query, context, chat)
formatted = format_output(output)
print(formatted)  # veya: return formatted
# print(output)
